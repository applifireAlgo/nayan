Ext.define('Testproject.testproject.web.com.test.controller.defaultdomain.EmployeeMainController', {
     ...
     deptLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#dept');
                    scope.sender.controller.assignAllComboData(scope.sender, 'dept', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#dept');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var compRef = this.view.down('#EmployeeTree');
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#Employee');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#EmployeeTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var Employee = this.view.down('#EmployeeForm');
               Employee.reset();
               var gridPanel = this.view.down('#EmployeeGrid');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Employee').down('#' + me.column.dataIndex + '').store;
     ...
     loadData: function(responseData) {
          this.view.down('#EmployeeForm').getViewModel().setData(responseData.response.data);
     },
