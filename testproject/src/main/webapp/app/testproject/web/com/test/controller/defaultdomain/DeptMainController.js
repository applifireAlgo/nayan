Ext.define('Testproject.testproject.web.com.test.controller.defaultdomain.DeptMainController', {
     extend: 'Testproject.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.DeptMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#DeptTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null && data instanceof Object) {
               var grid = this.view.down('#DeptGrid');
               grid.store.add(data);
               var tree = this.view.down('#DeptTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#Dept');
          var grid = this.view.down('#DeptGrid');
          var tree = this.view.down('#DeptTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#DeptGrid');
               ...
               var formPanel = this.getView().up().down("#Dept");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#DeptTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Dept').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#DeptGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.deptId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#Dept').getViewModel().setData(responseData.response.data);
     },
